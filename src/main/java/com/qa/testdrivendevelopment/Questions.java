...
public class Questions {
	...
	public int returnPosition(String input, char letter) {
		...
		if (a.indexOf(letter) != -1) {
			...
			int position = b.indexOf(letter) + 1;
			return position;
		} else {
			return -1;
		}
...
//		}
		...
		for (int i = 0; i < a && i < b; i++) {
			String sub1 = input1.substring(i, i + 1);
			String sub2 = input2.substring(i, i + 1);
			merged += sub1 + sub2;
		}
	...
	public int[] listGen() {
		...
		for (int i = 0; i < 5; i++) {
			int randomNum = (int) (Math.random() * 11) + 0;
			random[i] = randomNum * 2 + 100;
		}
		return random;
	...
	public boolean isPrime(int num) {
		...
		if (num / num == 1 && num / 1 == num && num / 2 != 4) {
			return true;
		} else {
			return false;
		}
